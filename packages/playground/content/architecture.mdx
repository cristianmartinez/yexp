...
Jext is built on a **stack-based bytecode virtual machine**. This architecture provides safe, fast, and deterministic expression evaluation.

## Compilation Pipeline

Jext uses a 3-stage compilation model:
...
All expressions run with three namespaces:
...
The VM can read from all three, but only mutate `state` (via assignment operations).
...
**Slow:** Parse `"data.items.price"` on every access
...
**Execution Flow:**
1. VM loads `data.items` from context
...
This design makes Jext ideal for:
