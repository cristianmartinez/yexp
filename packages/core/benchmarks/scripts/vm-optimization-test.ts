...
console.log(`Full evaluation:     ${fullTime.toFixed(2)}ms (${(fullTime / 100).toFixed(3)}µs per eval)`);
...
console.log(`Direct JavaScript:   ${jsTime.toFixed(2)}ms (${(jsTime / 100).toFixed(3)}µs per eval)`);
...
console.log(`Property access only: ${propTime.toFixed(2)}ms (${(propTime / 100).toFixed(3)}µs per access)`);
